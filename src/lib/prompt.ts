...
**阅读顺序（重要）**：请先读 JSON 顶部的 \`meta\`（口径明细）——尤其**太乙积年流派**${hasHuangji ? '与**皇极岁卦流派**' : ''}，不同流派积数/起卦口径不同、结果不可混用，请全程锁定该口径；再读 \`analysisContext\`（程序预先归集的断事要点，作为抓手）；最后深入 \`result\`${hasMingfa ? '、\`mingfa\`' : ''}${hasPlanets ? '、\`tenJing\`' : ''}${hasHuangji ? '、\`huangji\`' : ''} 等原始字段核对与展开。若含 \`kintaiyiPan\` 字段（kintaiyi 全解释盘，繁体中文键为上游《太乙統宗寶鑑》原样），其值宿断事/断法释文/釋格局/統運（卷十二）/卦象（卷十三）/军事诸卷/運籌博弈分析是断事的第一手釋文依据，请充分引用。${hasLife ? '若含 `kintaiyiLife` 字段（命法卷二十上游直出釋文：安命安身宮、飛祿飛馬黑符、十提金賦、十二宮星斷、雙星同宮論、諸星上中下三等），這是命理人事断的第一手釋文，命法部分请优先引用其原文。' : ''}若含 \`historyExamples\` 字段（该年份的古籍史例纪事与史載局數），请将盘面结论与史载事件互证并明确指出吻合或出入之处。若含 \`liuTimelines\` 字段（流年/流月/流日/流時/流分多期直卦序列，首期即起局时刻），请据以展开近期走势与流年推演——但须知这是**命法流卦（自出身卦挨步，随起局四柱含时辰呈相位变化，非全年恒定）**，规范且全年不变的年運卦是 \`result.yearGua\`（值年卦），论流年以值年卦为纲、流卦運相位为辅，勿把相位卦当作规范年卦；\`residence\` 为命主常居住地（不参与排盘，作人事断的方位地缘参照）；\`solarTime\` 标注了输入时间的解释时区与真太阳时校正明细；\`mingfa\` 中 yangjiuXingxian/bailiuXingxian 即大限（阳九/百六行限）全表，请结合当前虚岁点出所行之限。
