 ...
 * 真太阳时校正。
 *
 * 输入时间视为**所选地点民用时区**的墙上钟时间（排盘惯例：生辰/事件时刻即
 * 当地钟表时间，与操作者所在的浏览器时区无关）：
 *   真太阳时 = 输入时间 − 解释时区偏移 + 经度 × 4 分钟
 * 即校正量 offsetMinutes = 经度×4 − 解释时区偏移分钟。
 * - 城市模式（中国省市区）：解释时区恒为中国标准时间 UTC+8（偏移 480）。
 *   例：选潮州 116.62°E → 466.5 − 480 ≈ −13 分钟（6:10 → 5:57）。
 * - 当前时区（自动）：解释时区=浏览器时区，地点=该时区代表城市，两者自洽。
 *   例：东京时区（UTC+9）自动 → 东京 139.69°E → 558.8 − 540 ≈ +19 分钟。
...
export interface SolarTimeInfo {
  ...
  place?: string;
  longitude?: number;
  /** 输入时间的解释时区（自动模式=浏览器 IANA 时区；城市模式=中国标准时间） */
  timezone?: string;
  /** 解释时区偏移（分钟，东为正；城市模式下中国城市恒 +480） */
  tzOffsetMinutes?: number;
  offsetMinutes?: number;
  adjusted?: { year: number; month: number; day: number; hour: number; minute: number };
}

/** 中国标准时间（UTC+8）偏移：城市模式下输入时间按此解释 */
