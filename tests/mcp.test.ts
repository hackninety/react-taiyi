...
// 工具结果为 TOON 格式，解码后断言（与 JSON 语义等价）
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const parseT = (r: ToolResult): any => decode(textOf(r));
...
describe('taiyi MCP server', () => {
  ...
  it('taiyi_classics：目录 / 取卷（中文与数字卷号）/ 检索 / 分段越界', async () => {
    const client = await connect();
    // 目录
    const dir = parseT((await call(client, 'taiyi_classics')));
    expect(dir.书名).toBe('太乙金鏡式經');
    expect(dir.目录).toHaveLength(11); // 提要 + 十卷
    // 取卷（中文卷号）
    const v1 = parseT((await call(client, 'taiyi_classics', { chapter: '卷一' })));
    expect(v1.文).toContain('推上元積年');
    expect(v1.文).toContain('王希明');
    // 数字卷号 → 卷三
    const v3 = parseT((await call(client, 'taiyi_classics', { chapter: '3' })));
    expect(v3.卷).toBe('卷三');
    // 提要
    const ty = parseT((await call(client, 'taiyi_classics', { chapter: '提要' })));
    expect(ty.文).toContain('太乙金鏡式經十卷唐王希明撰');
    // 全书检索
    const q = parseT((await call(client, 'taiyi_classics', { query: '推上元積年' })));
  ...
  it('taiyi_history_examples：解析 md 表并按天文纪年匹配（stub fetch）', async () => {
    ...
    const client = await connect();
    const idx = parseT((await call(client, 'taiyi_history_examples')));
    expect(idx.条数).toBe(2);
    // 天文纪年 -2356 = 公元前 2357 年直记
    const hit = parseT((await call(client, 'taiyi_history_examples', { year: -2356 })));
