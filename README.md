...
- **勾选「皇极」解锁一元全跨度**（公元前 67016 — 公元 62583）：范围外太乙盘自动切换
  ...
  属现代拟推、非古历考据。验证锚点：该口径在 1582 年后与标准口径逐字段同盘（交叉验证锁定）；
  **公元前年计局数与年柱已同上游 kintaiyi（sxtwl 古历，原生支持公元前）经 67 条局數史例
  逐行对照一致**（`scripts/verify_examples.py` 生成基准，`tests/examples.test.ts` 锁定；
  农历为拟推月建、与古历朔望月不同，不在对照范围）；不勾选则维持 600–9999 默认范围
